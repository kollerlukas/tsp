...
abbreviation "cost_of_path\<^sub>c \<equiv> cost_of_path c"
...
lemma cost_of_path_sum: "cost_of_path\<^sub>c P = \<Sum>\<^sub># (image_mset c (mset (edges_of_path P)))"
  by (induction P rule: list012.induct) auto

lemma cost_of_path_sum_list: "cost_of_path\<^sub>c P = sum_list (map c (edges_of_path P))"
  by (induction P rule: list012.induct) auto

lemma cost_of_path_distinct_sum: 
  "distinct (edges_of_path P) \<Longrightarrow> sum c (set (edges_of_path P)) = cost_of_path\<^sub>c P"
...
lemma cost_of_path_app: "cost_of_path\<^sub>c (P @ [u,v]) = c {u,v} + cost_of_path\<^sub>c (P @ [u])"
...
proof (induction P rule: list012.induct)
  case (3 u v P)
  then have "cost_of_path\<^sub>c (u#v#P) = c {u,v} + cost_of_path\<^sub>c (rev P @ [v])"
    using "3.IH" by auto
  also have "... = cost_of_path\<^sub>c (rev (u#v#P))"
...
lemma cost_of_path_rotate: "cost_of_path\<^sub>c (u#P\<^sub>1 @ v#P\<^sub>2 @ [u]) = cost_of_path\<^sub>c (v#P\<^sub>2 @ u#P\<^sub>1 @ [v])" 
    (is "cost_of_path\<^sub>c ?P = cost_of_path\<^sub>c ?P'")
